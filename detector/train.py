...
def load_datasets(
    ...
    if fake_dataset == "TWO":
        ...
        fake_corpora = [
            Corpus(name, data_dir=data_dir) for name in ["xl-1542M", "xl-1542M-nucleus"]
        ]
        ...
        fake_corpora = [
            ...
            for name in ["xl-1542M", "xl-1542M-k40", "xl-1542M-nucleus"]
        ]
        fake_train = sum([corpus.train for corpus in fake_corpora], [])
        fake_valid = sum([corpus.valid for corpus in fake_corpora], [])
    else:
    ...
    train_loader = DataLoader(
        train_dataset, batch_size, sampler=Sampler(train_dataset), num_workers=0
    )

    validation_dataset = EncodedDataset(real_valid, fake_valid, tokenizer)
    validation_loader = DataLoader(
        validation_dataset, batch_size=1, sampler=Sampler(validation_dataset)
    )
...
def validate(
    model: nn.Module, device: str, loader: DataLoader, votes=1, desc="Validation"
):
    model.eval()
    ...
    records = [
        ...
        for record in tqdm(
            loader,
            desc=f"Preloading data ... {v}",
            disable=dist.is_available() and dist.get_rank() > 0,
        )
    ]
    records = [
        [records[v * len(loader) + i] for v in range(votes)] for i in range(len(loader))
    ]

    with tqdm(
        records, desc=desc, disable=distributed() and dist.get_rank() > 0
    ) as loop, torch.no_grad():
        for example in loop:
            losses = []
            logit_votes = []
    ...
    **kwargs,
    ...
    if world_size > 1:
        model = DistributedDataParallel(
            model, [rank], output_device=rank, find_unused_parameters=True
        )
    ...
    for epoch in epoch_loop:
        ...
        combined_metrics = _all_reduce_dict(
            {**validation_metrics, **train_metrics}, device
        )
